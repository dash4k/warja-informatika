<x-admin-dashboard title="Hasil Ujian Penjaluran">
    <section class="lg:right-[2.7%] mt-25 lg:ml-[20%] lg:mt-20 lg:w-7/9 min-h-screen w-5/6 max-w-[86%] mx-auto rounded-sm mainMainContentDashboard transition-all duration-300 flex flex-col gap-5" enctype="multipart/form-data">
        @if ($jumlahPerJalur->isEmpty() && $leaderboards->isEmpty())
            <div class="bg-red-100 border border-red-300 text-red-800 rounded p-4 text-center">
                Tidak ada data ujian terkini.
            </div>
        @else
            <form method="GET" action="{{ route('admin.penjaluran.showPenjaluran') }}" class="flex items-center gap-4">
                <label for="tahun" class="font-semibold">Pilih Angkatan:</label>
                <select name="tahun" id="tahun" onchange="this.form.submit()" class="border border-gray-300 rounded px-2 py-1">
                    @foreach($angkatanList as $angkatan)
                        <option value="{{ $angkatan }}" {{ $angkatan == $tahun ? 'selected' : '' }}>
                            20{{ $angkatan }}
                        </option>
                    @endforeach
                </select>
            ...
            <h1 class="font-poppins text-lg lg:text-xl font-bold">
                Hasil Ujian Angkatan 20{{ $tahun }}
            </h1>
